...
group_by(title, id, department, name)%>%
  summarize(u_grad_old = sum(u_grad))->combined_sep_old
...
```{r}
sep_new <- read_excel("new_data/class_enrollment_summary_by_term_9-22-2020 (1).xlsx", skip = 3) %>%
  clean_names()%>%
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
sep_new%>%
group_by(title, id, department, name)%>%
  summarize(u_grad = sum(u_grad))->combined_sep_new
...
-specifically define what we want to know
-recognize data that we probably don't need/won't be relevant
-be aware of assumptions that have been made

After we have a model, Temperance suggests we should:

-recognize the limitations of our models
-being humble about how our models might not be incomplete
-be aware that randomization does not always work
...
```{r}
inner_join(combined_sep_old, combined_sep_new)%>%
  mutate(enrollment_change = u_grad_old - u_grad)%>%
